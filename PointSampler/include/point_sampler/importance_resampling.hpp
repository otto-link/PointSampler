...
/**
 * @brief Generates a point set via importance resampling from a quasi-random oversampled grid.
 *
 * This function uses a Halton sequence to create an oversampled set of candidate points in the domain.
 * Each point is assigned a weight based on the provided density function.
 * A discrete distribution is then used to resample `count` points according to these weights.
 *
 ...
 * @tparam DensityFn     A callable with signature `T(const Point<T, N>&)` returning a non-negative density value
 * @param count          Number of points to return after resampling
 * @param oversampling_ratio Number of candidate points to generate as a multiple of `count`
 * @param axis_ranges    Axis-aligned bounding box defining the domain of the points
 * @param density_fn     Function mapping a point to a (non-negative) density value
 * @param seed           Optional seed to control the random number generator
 * @return std::vector<Point<T, N>> The resulting resampled point set
 *
 * ### Example
 * @code
